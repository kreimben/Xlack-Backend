...
class CallConsumer(AsyncJsonWebsocketConsumer):
        ...
        await self.send_json(
            {
                ...
                "format": json.dumps({"authorization": "{access_token}"}),
            }
        )
            ...
            if self.user is None:
                try:
                    self.user, _ = await sync_to_async(
                        AuthHelper.find_user_by_access_token
                    ...
                    await self.send_json(
                        {"success": False, "msg": f"{str(e)}"}, close=True
                    )
                except CustomUser.DoesNotExist:
                    await self.send_json(
                        {"success": False, "msg": "No such user"}, close=True
                    )
            else:
                await self.send_json(
                    {
    ...
    async def after_auth(self):
        ...
        await self.send_json(
            {"success": True, "msg": "Success to auth", "user_id": self.user.id}
        )
    ...
    async def from_client(self, content, **kwargs):
        ...
        if request is not None:
            ...
            # manage group call
                ...
                await self.send_json({"error": "invalid request", "format": info})
        else:
    ...
    async def call(self, content, kind):
        ...
        peer = self.user.id
        if target == None:
            await self.send_json({"error": "target is none, failed to start new call"})
            return
        if kind == "new":
    ...
    async def group_call(self, content, kind):
        ...
        if kind == "new":
            ...
            if group == None:
                await self.send_json({"error": "Group is none, failed to invite"})
                return
        elif kind == "accept":
            if self.group_channel == None:
                ...
                await self.send_json({"error": "Already in group, failed to join"})
                return
        elif kind == "reject":
    ...
    async def manage_exchange(self, content, **kwargs):
        ...
        target = content.get("target", None)
        group = content.get("group", None)
        candidate = content.get("candidate", None)
        ...
        if (
            request_type != "sdp.offer"
            and request_type != "sdp.answer"
            and request_type != "ice.candidate"
        ):
            await self.send_json({"error": "invaild type", "detail": request_type})
            return
        ...
        if candidate:
            response.setdefault("candidate", candidate)
        elif sdp:
            response.setdefault("sdp", sdp)

        await self.channel_layer.group_send(scope, response)
